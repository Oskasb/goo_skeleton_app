/*
 * goo-ui
 * Version: 4.0.3 - 2014-06-30
 */
angular.module("goo.ui", ["goo.ui.tpls", "goo.ui.checkbox","goo.ui.collapsible","goo.ui.colorInput","goo.ui.colorPicker","goo.ui.dialogs","goo.ui.dragdrop","goo.ui.dropdownToggle","goo.ui.dropzone","goo.ui.editFoldout","goo.ui.fallbackSrc","goo.ui.fileSelect","goo.ui.floatingLabel","goo.ui.input","goo.ui.inputSlider","goo.ui.keypressHelper","goo.ui.popup","goo.ui.utils","goo.ui.position","goo.ui.progressBar","goo.ui.propertyEdit","goo.ui.resize","goo.ui.select","goo.ui.sortable","goo.ui.tooltip"]);
angular.module("goo.ui.tpls", ["checkbox/checkbox.html","color_input/color-input.html","color_picker/color-picker.html","dialogs/dialog.html","edit_foldout/edit-foldout.html","input_slider/input-slider.html","popup/popup.html"]);

angular.module("checkbox/checkbox.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("checkbox/checkbox.html",
    "<div class=\"checkbox-checkmark icon-checked\"></div>");
}]);

angular.module("color_input/color-input.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("color_input/color-input.html",
    "<button\n" +
    "	ng-if=\"enabled !== undefined\"\n" +
    "	class=\"btn-icon-toggle icon-preview\"\n" +
    "	ng-class=\"{ active: enabled }\"\n" +
    "	ng-click=\"toggle()\"\n" +
    "	goo-tooltip=\"{{enabled ? 'Disable' : 'Enable'}}\"\n" +
    ">\n" +
    "</button>\n" +
    "<button\n" +
    "	ng-if=\"defaultColor\"\n" +
    "	class=\"reset icon-delete\"\n" +
    "	ng-click=\"reset()\"\n" +
    "	goo-tooltip=\"Reset to default color\"\n" +
    ">\n" +
    "</button>\n" +
    "<div\n" +
    "	class=\"color-display\"\n" +
    "	ng-click=\"openPicker($event)\"\n" +
    "	style=\"background-color:{{originalColor | rgbaArrayToCss}}\"\n" +
    "	goo-tooltip=\"Click to change color\"\n" +
    ">\n" +
    "</div>");
}]);

angular.module("color_picker/color-picker.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("color_picker/color-picker.html",
    "<div class=\"saturation\">\n" +
    "	<canvas height=\"120\" width=\"120\"></canvas>\n" +
    "	<span style=\"left: {{ satPos[0].toFixed(2) }}px; top: {{ satPos[1].toFixed(2) }}px;\"></span>\n" +
    "</div>\n" +
    "<div class=\"hue\">\n" +
    "	<canvas height=\"150\" width=\"25\"></canvas>\n" +
    "	<span style=\"top: {{ huePos.toFixed(2) }}px\"></span>\n" +
    "</div>\n" +
    "<div class=\"hex\">\n" +
    "	<label>Hex</label>\n" +
    "	<input type=\"text\" ng-model=\"hex\" ng-change=\"updateFromHex()\" ng-blur=\"gooBlur()\"/>\n" +
    "</div>\n" +
    "");
}]);

angular.module("dialogs/dialog.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("dialogs/dialog.html",
    "<div ng-repeat=\"dialog in dialogs\"\n" +
    "	class=\"dialog-wrapper\"\n" +
    "	id=\"dialog_{{dialog.id}}\"\n" +
    "	ng-click=\"close(null); $event.stopPropagation()\"\n" +
    "	ng-show=\"dialog.show\">\n" +
    "	<div ng-if=\"dialog.pluginName != 'default'\" goo-plugin=\"dialog.pluginName\"></div>\n" +
    "\n" +
    "	<div ng-if=\"dialog.pluginName == 'default' && dialog.type != 'embedded'\" class=\"dialog\">\n" +
    "		<header class=\"{{dialog.type}}\"><h2 ng-bind=\"dialog.title\"></h2></header>\n" +
    "		<div class=\"dialog-content\">\n" +
    "			<p ng-bind-html=\"toTrusted(dialog.message)\"></p>\n" +
    "			<ul ng-show=\"dialog.listItems.length>0\">\n" +
    "				<li ng-repeat=\"item in dialog.listItems\" ng-bind=\"item\"></li>\n" +
    "			</ul>\n" +
    "\n" +
    "			<p class=\"contact-msg\" ng-show=\"dialog.type=='error'\">\n" +
    "				Did you find a bug? Please email us at\n" +
    "				<a\n" +
    "					href=\"mailto:support@gootechnologies.com\"\n" +
    "					target=\"_blank\"\n" +
    "				>\n" +
    "					support@gootechnologies.com</a> or hit the\n" +
    "				<a\n" +
    "					href=\"http://www.gootechnologies.com/forum\"\n" +
    "					title=\"Goo Forums\"\n" +
    "					target=\"_blank\"\n" +
    "				>\n" +
    "					Goo Forums</a> to get help!\n" +
    "			</p>\n" +
    "		</div>\n" +
    "		<footer>\n" +
    "			<button ng-repeat=\"button in dialog.buttons\" ng-click=\"close(button.action(), $event)\" class=\"{{button.cssClass}}\">{{button.label}}</button>\n" +
    "		</footer>\n" +
    "	</div>\n" +
    "\n" +
    "	<div ng-if=\"dialog.pluginName == 'default' && dialog.type == 'embedded'\" class=\"dialog wide\">\n" +
    "		<header><h2 ng-bind=\"dialog.title\"></h2></header>\n" +
    "		<div class=\"dialog-content\">\n" +
    "			<iframe ng-src=\"{{dialog.source || ''}}\" style=\" width: {{(dialog.width !== undefined)?dialog.width+'px':'auto'}}; height: {{(dialog.height !== undefined)?dialog.height+'px':'auto'}}\" frameBorder=\"0\" webkitAllowFullScreen mozallowfullscreen allowFullScreen>\n" +
    "			</iframe>\n" +
    "		</div>\n" +
    "		<footer>\n" +
    "			<button ng-repeat=\"button in dialog.buttons\" ng-click=\"close(button.action(), $event)\">{{button.label}}</button>\n" +
    "		</footer>\n" +
    "	</div>\n" +
    "</div>\n" +
    "");
...
angular.module("edit_foldout/edit-foldout.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("edit_foldout/edit-foldout.html",
    "<label title=\"{{labelTip}}\">\n" +
    "    {{label}}\n" +
    "</label>\n" +
    "<button class=\"remove-button\" ng-click=\"doDelete()\" ng-show=\"delete && delete != ''\" title=\"Delete\"></button>\n" +
    "<button class=\"edit-button\" ng-click=\"folded=!folded\"></button>\n" +
    "<div class=\"settings-content\" ng-hide=\"folded\" ng-transclude>\n" +
    "</div>\n" +
    "");
}]);

angular.module("input_slider/input-slider.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("input_slider/input-slider.html",
    "<div class=\"left\" style=\"width: calc({{sliderPos*100}}% - {{sliderPos}} * 16px);\"></div>\n" +
    "<div class=\"ball\"></div>\n" +
    "<div class=\"right\"></div>");
}]);

angular.module("popup/popup.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("popup/popup.html",
    "<div tabindex=\"-1\"\n" +
    "	 class=\"popup fade inverse {{ placement }} {{ popupClass }}\"\n" +
    "	 ng-class=\"{in: animate}\"\n" +
